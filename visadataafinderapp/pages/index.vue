...
<div class="bg-slate-50 border border-slate-200 rounded-lg p-6">
  <div class="flex items-start space-x-4">
    <div class="bg-white p-2 rounded-lg shadow-sm">
      <Icon name="mdi-calendar-refresh" class="text-sky-500" size="28" />
    </div>
    ...
    <div class="flex-1 space-y-2">
      ...
      <!-- Period Status -->
      <div v-if="formatRollingPeriod"
           class="border-t border-slate-200 pt-3 mt-3"
      >
        <template v-if="!exitDate">
          <div class="flex items-center text-slate-600">
            <Icon name="mdi-information" size="20" class="mr-2" />
        ...
        <template v-else>
          ...
          <div class="flex items-center space-x-2 mb-2">
            <span
              :class="{
                'bg-red-100 text-red-700': formatRollingPeriod.isExpired,
                'bg-emerald-100 text-emerald-700': !formatRollingPeriod.isExpired
              }"
              class="inline-flex items-center px-2.5 py-0.5 rounded-full text-sm font-medium"
            >
              <Icon
                :name="formatRollingPeriod.isExpired ? 'mdi-alert-circle' : 'mdi-check-circle'"
                size="16"
                class="mr-1"
              />
              {{ formatRollingPeriod.isExpired ? 'Period Expired' : 'Period Active' }}
            </span>
          </div>
          ...
          <div class="flex flex-col sm:flex-row sm:items-center sm:space-x-4 text-sm">
            <span :class="{
              'text-red-600': formatRollingPeriod.isExpired,
              'text-emerald-600': !formatRollingPeriod.isExpired
            }">
              <strong>
                {{ formatRollingPeriod.isExpired ? 'Expired' : 'Ends in' }}:
              </strong>
              {{ formatRollingPeriod.formattedPeriod }}
            </span>
            <span class="text-slate-500 mt-1 sm:mt-0">
